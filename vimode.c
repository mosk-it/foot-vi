...
// scrolled out.
// TODO (kociap): C+b and C+f move the view back to the bottom.
...
delete_chars_from_search_backward(struct vimode_search *const search, int count)
{
  if (search->cursor > 0) {
    ...
    memmove(&search->buf[search->cursor - count], &search->buf[search->cursor],
                                             ...
                                             int count)
{
  if (search->cursor < search->len) {
    count = count < search->len - search->cursor ? count
                                                 : search->len - search->cursor;
    memmove(&search->buf[search->cursor], &search->buf[search->cursor + count],
                                          ...
                                          bool *search_string_changed)
{
  ...
  *search_string_changed = false;
  ...
  case BIND_ACTION_VIMODE_SEARCH_DELETE_PREV_WORD: {
    ...
    bool const deleted = delete_chars_from_search_backward(search, distance);
    *search_string_changed = deleted;
  } break;
  ...
  case BIND_ACTION_VIMODE_SEARCH_DELETE_TO_START: {
    bool const deleted = delete_chars_from_search_backward(search, search->len);
    *search_string_changed = deleted;
  } break;

  case BIND_ACTION_VIMODE_SEARCH_DELETE_TO_END: {
    bool const deleted = delete_chars_from_search_forward(search, search->len);
    *search_string_changed = deleted;
  } break;
